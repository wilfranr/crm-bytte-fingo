...
const CONSOLIDADO_CASB_PATH = path.join(PROCESSED_DIR, "Consolidado_casb.xlsx");
...
export const procesarArchivoCasb = (req, res) => {
  if (!req.file) {
  ...
  try {
    ...
    const transaccionesData = xlsx.utils.sheet_to_json(transaccionesSheet, {
      header: 1,
      range: 7,
    });
    ...
    let consolidadoWb;
    let baseData = [];
    if (fs.existsSync(CONSOLIDADO_CASB_PATH)) {
      consolidadoWb = xlsx.readFile(CONSOLIDADO_CASB_PATH);
    ...
    const documentosEnBase = new Set(
      baseData.map((row) => String(row.DOCUMENTO)),
    );
    ...
    exitosas.forEach((row) => {
      ...
      if (!documentosEnBase.has(doc) && !documentosExitososUnicos.has(doc)) {
        nuevosDocumentos.push({
          DOCUMENTO: doc,
          MES:
            getSpanishMonthName(previousMonthDate) +
            " " +
    ...
    if (consolidadoWb.SheetNames.includes(monthSheetName)) {
      ...
      monthSheet = xlsx.utils.aoa_to_sheet([
        ["Enrolados Facturables"],
        ["No Enrolados Facturables"],
      ]);
    ...
    xlsx.utils.sheet_add_aoa(monthSheet, [[totalNoEnrolados]], {
      origin: "B2",
    });

    xlsx.writeFile(consolidadoWb, CONSOLIDADO_CASB_PATH);

    res.download(CONSOLIDADO_CASB_PATH, "Consolidado_casb.xlsx", (err) => {
      if (err) {
        res
          .status(500)
          .send({
            message: "Error al descargar el archivo consolidado.",
            error: err.message,
          });
      } else {
        // Limpiar el archivo subido después de procesarlo y descargarlo
    ...
    res
      .status(500)
      .send({ message: "Error al procesar el archivo.", error: error.message });
  } finally {
    // Si hay un error antes de la descarga, limpiar el archivo subido aquí
